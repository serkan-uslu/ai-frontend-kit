...
const extractTextContent = (node: React.ReactNode): string => {
  ...
  if (isValidElement(node)) {
    // Use a more specific type for React elements with children
    const elementWithChildren = node as React.ReactElement<{
      children?: React.ReactNode;
    }>;
    return extractTextContent(elementWithChildren.props.children || "");
  }
  return "";
};
...
const components: Partial<Components> = {
  ...
  img: ({ alt, src, ...props }: React.ImgHTMLAttributes<HTMLImageElement>) => {
    // Handle src properly for Next.js Image component
    ...
    return (
      <div className="my-4 w-full">
        <Image
          ...
          height={350}
          className={cn(
            "rounded-md max-w-full h-auto object-cover",
            props.className,
          )}
